...
=== approaches to ${O} (${V.length}) ===`);const H=re.get(O);H&&console.log(`   box at (${Math.round(H.x)},${Math.round(H.y)}) h=${Math.round(H.height)}`),V.forEach(K=>console.log("   "+K))}},[B,Se,re]);const ae=b.useMemo(()=>{const A=new Map;if(!B)return A;for(const O of B.edges){const V=Se.get(O.id);if(!V||V.storageDirection==="flipped"||q.has(O.id)||pi(l,ln(O.sections))<=0)continue;const H=ce(V)===V.source?V.target:V.source,K=`${H}|${H===V.source?"out":"in"}`,Y=Q.get(K);if(!Y)continue;const M=V.type==="ownership",_=ve.get(V.source)==="context"||ve.get(V.target)==="context",W=x.edgeColors.get(O.id),Z=A.get(K);A.set(K,Z?{...Z,isOwn:Z.isOwn||M,dimmed:Z.dimmed&&_,edgeIds:[...Z.edgeIds,O.id],...Z.color?.text===W?.text?{}:{color:void 0}}:{...Y,isOwn:M,dimmed:_,edgeIds:[O.id],...W?{color:W}:{}})}return A},[B,Se,Q,l,ve,x,q]),we=b.useMemo(()=>new Set(x.nodes.map(A=>A.id)),[x]),Le=b.useCallback(A=>!!i&&A.channel!=="plain"&&!A.isLoop&&!we.has(A.range),[i,we]),Ne=b.useRef(null),dt=b.useRef(null),ft=b.useRef(void 0),pt=b.useMemo(()=>{const A=new Map,O=new Map;for(const V of x.edges){O.set(V.id,[V.source,V.target]);for(const H of[V.source,V.target])A.set(H,[...A.get(H)??[],V.id])}return{nodeEdges:A,edgeEnds:O}},[x]),U=b.useRef(pt);U.current=pt;const J=b.useCallback(A=>{ft.current=A,dt.current===null&&(dt.current=requestAnimationFrame(()=>{dt.current=null;const O=ft.current;ft.current=void 0;const V=Ne.current,H=F.wrapperRef.current;if(O===void 0||!V||!H)return;let K=null,Y=null;if(O){const{nodeEdges:_,edgeEnds:W}=U.current;if(O.kind==="node"){K=new Set(_.get(O.id)??[]),Y=new Set([O.id]);for(const Z of K)for(const ue of W.get(Z)??[])Y.add(ue)}else K=new Set([O.id]),Y=new Set(W.get(O.id)??[])}const M=(_,W,Z)=>{_.style.filter=W===null||W?"":`opacity(${Z})`};V.querySelectorAll("path[data-edge-id]").forEach(_=>{const W=_.dataset.edgeId??"",Z=K?K.has(W):null;M(_,Z,.38),_.style.strokeWidth=Z?String(_.dataset.channel==="reference"?Im:Gl):""}),V.querySelectorAll("path[data-arrowhead]").forEach(_=>{const W=(_.dataset.arrowhead??"").split(" ");M(_,K?W.some(Z=>K.has(Z)):null,.08)}),H.querySelectorAll("[data-node-id]").forEach(_=>{M(_,Y?Y.has(_.dataset.nodeId??""):null,.25)})}))},[]);b.useEffect(()=>J(null),[x,B,J]);const pe=A=>m(O=>{const V=new Set(O);return V.has(A)?V.delete(A):V.add(A),V}),me=A=>{vr("dir",A),c(A)},Yt=A=>{vr("merge",A),u(A)},Xt=e.getConceptLabel("attribute",!0).toLowerCase(),qe=A=>`px-2 py-0.5 text-xs rounded border ${A?"border-blue-500 bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-300":"border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 hover:bg-gray-100 dark:hover:bg-slate-700"}`;return h.jsxs("div",{className:"relative w-full h-full",children:[h.jsxs("div",{"data-pan-ignore":!0,className:"absolute top-2 right-2 z-10 flex gap-1 items-center",children:[Me&&h.jsxs("div",{className:`mr-2 flex items-center gap-2 rounded px-2 py-1
                          ...
                          bg-white/80 dark:bg-slate-900/80 shadow-sm`,children:[h.jsx("span",{className:`inline-block h-3 w-3 animate-spin rounded-full
                             ...
                             dark:border-slate-600 dark:border-t-slate-300`}),"Computing layout…"]}),o&&h.jsxs(h.Fragment,{children:[h.jsx("button",{className:qe(r),title:r?"Hide owners: show only what you selected":"Show every owner up to the root (can pull in most of the schema)",onClick:o,children:"⇱ roots"}),h.jsx("span",{className:"w-px h-4 bg-gray-300 dark:bg-slate-600 mx-1"})]}),h.jsx("button",{className:qe(a==="RIGHT"),title:"Layout left to right",onClick:()=>me("RIGHT"),children:"LR"}),h.jsx("button",{className:qe(a==="DOWN"),title:"Layout top down",onClick:()=>me("DOWN"),children:"TB"}),h.jsx("span",{className:"w-px h-4 bg-gray-300 dark:bg-slate-600 mx-1"}),h.jsx("button",{className:qe(l==="near"),title:"Merge converging edges near the node (~40px)",onClick:()=>Yt("near"),children:"⋙"}),h.jsx("button",{className:qe(l==="far"),title:"Merge converging edges early (~120px)",onClick:()=>Yt("far"),children:"⋙⋙"}),h.jsx("button",{className:qe(l==="bend"),title:"Merge at ELK's last corner",onClick:()=>Yt("bend"),children:"⌙"}),h.jsx("button",{className:qe(l==="off"),title:"No merging — every edge runs to its own port",onClick:()=>Yt("off"),children:"≡"}),h.jsx("span",{className:"w-px h-4 bg-gray-300 dark:bg-slate-600 mx-1"}),[["+",()=>F.zoomBy(1.3),"Zoom in"],["−",()=>F.zoomBy(1/1.3),"Zoom out"],["1:1",()=>F.applyZoom(1),"Reset zoom"],["⛶",()=>F.zoomToFit(),"Fit to view"]].map(([A,O,V])=>h.jsx("button",{onClick:O,title:V,className:qe(!1),children:A},A))]}),h.jsx("div",{ref:F.containerRef,"data-graph-direction":a,className:"w-full h-full overflow-auto cursor-grab",children:h.jsx("div",{ref:F.spacerRef,children:h.jsx("div",{ref:F.wrapperRef,className:"relative",children:R&&h.jsxs(h.Fragment,{children:[h.jsxs("svg",{ref:Ne,className:"absolute top-0 left-0 pointer-events-none",width:S,height:z,children:[h.jsxs("defs",{children:[(()=>{const A=An("forward"),{d:O,...V}=A;return h.jsx("marker",{id:d("arrow-own"),...V,children:h.jsx("path",{d:O,fill:Ke.ownFwd})})})(),(()=>{const{d:A,...O}=An("backward");return h.jsx("marker",{id:d("arrow-own-back"),...O,children:h.jsx("path",{d:A,fill:Ke.ownBkwd})})})(),(()=>{const{d:A,...O}=An("forward",Nm);return h.jsx("marker",{id:d("arrow-assoc"),...O,children:h.jsx("path",{d:A,fill:Ke.association})})})()]}),h.jsxs("g",{transform:`translate(${gt}, ${gt})`,style:{opacity:fe?1:0,transition:`opacity ${cm()}ms`},children:[[...ae].map(([A,O])=>h.jsx("path",{"data-arrowhead":O.edgeIds.join(" "),d:Yp(O.base,O.dir,jm,kr),fill:O.color?.text??Tr(O.isOwn,!1),opacity:O.dimmed?.4:1,style:{transition:`filter ${un()}ms`}},`head-${A}`)),(B?.edges??[]).map(A=>{const O=Se.get(A.id);if(!O)throw new Error(`Routed edge ${A.id} missing from view model`);const V=O.storageDirection==="flipped",H=ce(O)===O.source?O.target:O.source,K=V||q.has(A.id)?void 0:Q.get(`${H}|${H===O.source?"out":"in"}`),Y=ne.get(A.id),M=!!K&&pi(l,Y??ln(A.sections))>0,_=O.type!=="ownership",W=M?A.sections:$m(A.sections,mr(_?"association":"own-fwd")),Z=_?Fm(W,mr("association")):W,ue=Y??ln(Z),Ee=Qn=>Gp(Qn,Om),Ve=pi(l,ue),tt=K&&Ve>0?qp(ue,K.base,Ve,Ee):Ee(ue);if(!tt)return null;const Qe=O.type==="ownership",mt=ve.get(A.source)==="context"||ve.get(A.target)==="context",Zt=M?void 0:Qe?V?"arrow-own-back":"arrow-own":"arrow-assoc";return h.jsxs("g",{children:[h.jsx("path",{"data-edge-id":A.id,"data-channel":Qe?"ownership":"reference",d:tt,fill:"none",opacity:mt?.4:1,stroke:x.edgeColors.get(A.id)?.text??Tr(Qe,V),strokeWidth:Qe?zl:Vm,strokeDasharray:Qe?void 0:ie.dash,markerEnd:Zt?`url(#${d(Zt)})`:void 0,markerStart:!Qe&&!M?`url(#${d("arrow-assoc")})`:void 0,style:{transition:`filter ${un()}ms, stroke-width ${un()}ms`}}),h.jsx("path",{d:tt,fill:"none",stroke:"transparent",strokeWidth:11,style:{pointerEvents:"stroke"},onMouseEnter:()=>J({kind:"edge",id:A.id}),onMouseLeave:()=>J(null)})]},A.id)})]})]}),h.jsx(Df,{initial:!1,children:x.nodes.map(A=>{const O=re.get(A.id);if(!O)return null;const V=A.role==="context",H=O.x+gt,K=O.y+gt,Y={duration:cn(C.has(A.id)?0:Rl()),ease:rm};return h.jsxs(Bp.div,{initial:{opacity:0,x:H,y:K},animate:{opacity:V?Cm:1,x:H,y:K},exit:{opacity:0,transition:{duration:cn(br())}},transition:{x:Y,y:Y,opacity:{duration:cn(br()),delay:cn(lm())}},"data-node-id":A.id,"data-help-id":Lc(A),"data-pan-ignore":!0,"data-pinned":T.has(A.id)?"":void 0,onPointerDown:M=>Pe(A.id,M),onClick:()=>{if(X.current){X.current=!1;return}n?.(A.members.length?A.label:A.id)},onMouseEnter:()=>J({kind:"node",id:A.id}),onMouseLeave:()=>J(null),className:`absolute rounded-md text-xs bg-white dark:bg-slate-800 cursor-pointer ${V?"border border-dashed border-gray-400 dark:border-slate-500":T.has(A.id)?"border-2 border-amber-500 dark:border-amber-400 shadow-md":"border-2 border-slate-500 dark:border-slate-400 shadow-md"}`,style:{width:Ce,height:A.height,transition:`filter ${un()}ms`},children:[h.jsxs("div",{className:"flex items-center gap-1 px-2 rounded-t-[4px] bg-slate-700 dark:bg-slate-700 text-white border-b border-slate-800 dark:border-slate-600",style:{height:ct},children:[h.jsx("span",{className:`font-semibold truncate ${A.abstract?"italic":""}`,title:A.description||A.id,children:A.label}),h.jsxs("span",{className:"ml-auto flex gap-1 shrink-0",children:[A.members.length>0&&h.jsxs("span",{title:`${A.members.length} classes that are a ${A.label}, merged into one box`,className:"text-[9px] px-1 rounded bg-sky-100 dark:bg-sky-900 text-sky-800 dark:text-sky-200",children:["⑃ ",A.members.length]}),A.isaParents.map(M=>h.jsxs("span",{title:`is-a ${M}`,className:"text-[9px] px-1 rounded bg-sky-100 dark:bg-sky-900 text-sky-800 dark:text-sky-200",children:["⊳ ",M]},M)),A.subclassCount>0&&A.members.length===0&&h.jsxs("span",{title:`${A.subclassCount} subclasses shown`,className:"text-[9px] px-1 rounded bg-sky-100 dark:bg-sky-900 text-sky-800 dark:text-sky-200",children:["▷ ",A.subclassCount]}),(()=>{const _=(A.members.length?A.members.map(W=>W.id):[A.id]).filter(W=>t.has(W));return _.length?h.jsx("button",{"data-dismiss":A.id,"data-help-id":"node-dismiss",title:_.length>1?`Remove all ${_.length} selected classes in ${A.label}`:`Remove ${A.label} from the canvas`,onClick:W=>{W.stopPropagation(),_.forEach(Z=>s?.(Z))},className:`text-[10px] leading-none px-1 rounded text-gray-400
                                     ...
                                     bg-sky-50/60 dark:bg-sky-950/30`,style:{height:$s},children:h.jsx(mm,{label:A.label,rows:A.relationRows,onAdd:M=>i?.(M),onRemove:M=>s?.(M),onInspect:n,colorOf:p,slotOrder:A.allRows.map(M=>M.slot),parentOf:M=>e.getClassSummary(M)?.parentId})}),A.rows.map(M=>M.header?h.jsx("div",{"data-no-drag":!0,"data-help-id":Oc(M.header.id),title:`${M.header.label} — is a ${A.label}; click for details`,onClick:_=>{_.stopPropagation(),n?.(M.header.id)},className:`flex items-center px-2 text-[10px] font-semibold
