...
| 5b | **Canvas enter/exit — boxes DONE, edges deferred.** Boxes moved to `motion/react` 2026-09-09 (`<AnimatePresence>`; the hand-rolled retention is gone) and Siggie has checked them in the browser. **Edges still snap** — they fade out and back in on `EDGE_ARRIVE_MS`; animating their geometry is the remaining half and Siggie chose to put it off. When picked up: the design (pad ELK's route points to a common count, interpolate `d` with `motion.path`) is in [CANVAS_TRANSITIONS.md §Edges](CANVAS_TRANSITIONS.md#edges); edge ids are index-based (`edge-${idx}` in containmentGraph.ts) so a persisting edge needs a STABLE key first. ⚠️ d3 is closed entirely. | ? | [CANVAS_TRANSITIONS](CANVAS_TRANSITIONS.md) |
