...
| 8 | **Migrate positioning to CSS anchor positioning.** Deletes the 250ms poll, the flip/clamp, the `EST_H` guess and the stale-hint bug — and, with them, the drag blocker (8b). **Smaller than it has been treated as**: counted 2026-09-08, **44 of the 48 resolver anchors in the live content are `node-box:`**, which is one `anchor-name` at one render site, and `slot-row` (the awkward attribute-pair kind) is used by NOTHING. ⚠️ Support re-checked 2026-09-08: Baseline **newly** available (Jan 2026), not widely — Chrome 125+, Safari 18.2+, Firefox 147+, ~91% of traffic — so it wants an `@supports` guard with the measured path as fallback. **Unblocked** — S3b's resolvers landed. ⚠️ **`EST_H` has a live symptom**, seen 2026-09-07: a tall popover over a populated canvas renders with its bottom cut off. `EST_H = 260` (`HelpLayer.tsx`) is a hardcoded GUESS at popover height used for clamping, so anything taller is positioned as if it were 260px and runs off the bottom. The `maxHeight`/scroll path already handles real height; the clamp arithmetic does not. Fixable on its own if this migration stays parked. ⚠️ **Of the three hardcoded estimates, only ONE belongs to this task** — corrected 2026-09-08. `EST_H`/`estHeight` guess the popover's own height in order to clamp with it, and CSS anchoring deletes them outright. `CHAR_W`/`LINE_H` (the width curve) and `navMinWidth` are about the popover's own SIZE, which no amount of anchoring answers — `autoWidth` is a deliberate design lever (pick a width so prose is not a tall thin column), not a measurement of anything on screen, and it stays. See HELP_PACKAGE_PLAN §1 "What goes, what stays". **Decided (Siggie): current browsers only** — no `@supports` guard and no retained fallback, so the measured path is deleted rather than demoted. | ~0.5 day | [HELP_PACKAGE_PLAN §1](HELP_PACKAGE_PLAN.md#1-migrate-positioning-to-css-anchor-positioning) |
| 8b | **Overlays: one model, draggable and resizable.** The detail drawer is an in-flow flex column while the legend and cases float, so the legend covers the drawer by construction. Make the drawer an overlay and make overlays draggable/resizable. **Do task 8 first after all** — dragging *could* be unblocked on its own by computing the popover's position once instead of every poll tick, but that keeps the measure-then-position machinery alive and adds state to it. Siggie, 2026-09-08: *"finding the screen position of one thing and then using that to set the position of another thing is kludgy and css should make it so we don't have to do that."* Task 8 deletes the machinery and dragging falls out for free. | [BACKLOG §Overlays](BACKLOG.md#overlays-one-model-draggable-and-resizable) |
