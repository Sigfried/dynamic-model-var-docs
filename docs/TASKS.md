...
The active work. The plan is [TOURS_AND_CONTENT.md](TOURS_AND_CONTENT.md).
 ...
 | `help-finish-authoring/ownership` | **The Ownership tour.** Siggie has started rewriting it in `help-content.md` into a version that makes more sense to them (the `why-ownership` entry and its beats, 2026-09-11); the most to do is here. Consider [`ownership-rules-declarative`](#now--finish-the-tours) and [`drop-association`](#now--finish-the-tours) first — both change what the tour has to explain. | [help-content.md](../src/explore/help-content.md) |
 ...
 | `drop-association` | **Get rid of `association` edges.** Both members of `ASSOCIATION_SLOTS` become ordinary ownership: `Specimen.related_document` → Rule 1 `own-fwd` (Document connects to nothing but Specimen and its own focus, so a document belongs to its specimen); `SpecimenStorageActivity.container` → Rule 1 `own-fwd` (no clear direction between container and activity, but not important enough for a whole edge type). And **`Specimen.contained_in` should point forward** — a container has no independent existence from its specimen, so it is an Exception 2a target, not an owner. That should also dissolve the `Specimen → SpecimenStorageActivity → SpecimenContainer → Specimen` cycle that `association` existed to break; verify. **Decided 2026-09-11 — see [OWNERSHIP_RULES_PLAN.md](OWNERSHIP_RULES_PLAN.md):** the classification flip happens now (three slots, zero association edges, tour unblocked); the `association` machinery is DELETED only after `ownership-rules-declarative`, because it is the only live test case for making edge kinds configurable. Restoring it must be possible from the spec, not from git history. | [containmentGraph.ts](../src/models/containmentGraph.ts) |
 | `ownership-rules-declarative` | **One declarative place for every classification rule and exception.** [OWNERSHIP_CLASSIFICATION §Where it lives](OWNERSHIP_CLASSIFICATION.md#where-it-lives) makes it look like a big mess: the rules are code in `classifySlotEdge`, the exceptions are five hand-curated sets keyed by slot name, the rule text is a third thing, and the legend groups by yet another. (May be worth looking at `override-site-check` and [BACKLOG §Config rot](BACKLOG.md#hand-curated-config-rot)). **IMPLEMENTED 2026-09-11, UNREVIEWED** — [`src/models/ownershipRules.ts`](../src/models/ownershipRules.ts) is one ordered rule array plus a verdict table carrying the drawn appearance; `OWNERSHIP_RULE_TEXT` and `EDGE_STYLE.kinds` are now projections of it. The acceptance criterion ("can it express `association` as config?") is asserted by [`ownershipRules.test.ts`](../src/test/ownershipRules.test.ts). **Siggie has not read [OWNERSHIP_RULES_PLAN.md](OWNERSHIP_RULES_PLAN.md) or this code** and may want either reversed — it is its own commit for that reason. Blocks step 3 of [`drop-association`](#now--finish-the-tours). | [OWNERSHIP_CLASSIFICATION.md](OWNERSHIP_CLASSIFICATION.md) |
...
## Then — cleanups with a known shape
  ...
  | `override-site-check` | **A sync check that each `containmentGraph` override slot name still has exactly one site.** The sets are keyed by SLOT NAME, not `(class, slot)`, and every member happening to occur at one class is **luck, not design**.                                    | [BACKLOG §Config rot](BACKLOG.md#hand-curated-config-rot)                                                                    |
 ...
 | `ownership-doc-cleanup` | **Clean up OWNERSHIP_CLASSIFICATION.md.** Lots of it. Do it AFTER `drop-association` and `ownership-rules-declarative`, or the doc gets rewritten twice. | [OWNERSHIP_CLASSIFICATION.md](OWNERSHIP_CLASSIFICATION.md) |
