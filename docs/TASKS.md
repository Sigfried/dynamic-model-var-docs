...
| 5b | **Redo canvas enter/exit with `motion/react`.** The zoom/movement half of the 2026-09-09 transition work is done and good; the enter/exit half is hand-rolled scaffolding (`outgoingRef`, `shownVmRef`, `departing`, `arrived`, `setRetired`, a retirement timer) that keeps a departing box alive because its `vm` entry is gone. Departing boxes were still not leaving correctly when the session ended — **assume broken**. `<AnimatePresence>` is the keep-it-alive-while-it-leaves primitive, and `layout`/`layoutId` cover most of the hand-rolled movement. Rip the scaffolding out rather than repairing it. ⚠️ d3-transition/d3-selection stay rejected (they fight React for DOM ownership); `d3-interpolate` alone is still live for edge geometry. | ? | [WORKLOG 2026-09-09 (later)](../WORKLOG.md) |
| 5c | **Read `temp/elk-timings.jsonl` and settle `SPINNER_DELAY_MS`.** Instrumentation is committed and dev-only but has never been read — it needs a dev-server restart to load the vite plugin. Delete `scripts/elkTimingPlugin.ts` + `src/explore/graph-core/elkTiming.ts` once the number is set. | 15 min | [anim.ts](../src/explore/graph-core/anim.ts) |
