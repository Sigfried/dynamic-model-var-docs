...
| # | Task | Est. | Where |
|---|---|---|---|
| 5 | ~~**Re-render regression.**~~ **Closed 2026-09-09.** The canvas half (every box unmounting on every click) was fixed in `3c5e2f1`; the panel half no longer reproduces — Siggie: *"i'm not seeing re-render behavior anymore."* Its four suspects were never measured; if it comes back, measure (render counter or the React profiler) before believing any of them. | — | [WORKLOG 2026-09-09](../WORKLOG.md) |
| 5b | **Redo canvas enter/exit with `motion/react`.** The zoom/movement half of the 2026-09-09 transition work is done and good; the enter/exit half is hand-rolled scaffolding (`outgoingRef`, `shownVmRef`, `departing`, `arrived`, `setRetired`, a retirement timer) that keeps a departing box alive because its `vm` entry is gone. Departing boxes were still not leaving correctly when the session ended — **assume broken**. Rip the scaffolding out rather than repairing it. The design — constraints already settled, the staged-vs-intermediate-layout question, edge transition ideas, and Siggie's notes from an outside session — is in [CANVAS_TRANSITIONS.md](CANVAS_TRANSITIONS.md). ⚠️ d3 is closed entirely, including `d3-interpolate`. | ? | [CANVAS_TRANSITIONS](CANVAS_TRANSITIONS.md) |
| 5c | **Stop killing a warm ELK worker; then delete the timing instrumentation.** `temp/elk-timings.jsonl` was read 2026-09-09 (347 runs): p50 153ms, p90 322ms, floor ~100ms even for 2-node graphs — and **every run was cold**, because `useGraphLayout`'s effect cleanup calls `engine.cancel()` on every spec change whether or not a run is in flight, so every click pays worker startup. Terminate only when a run is actually pending. That likely settles `SPINNER_DELAY_MS` at its current 200 (26% of runs exceed it today; almost none should once warm). Then delete `scripts/elkTimingPlugin.ts` + `src/explore/graph-core/elkTiming.ts` and the `recordElkTiming` call in [elkLayout.ts](../src/explore/graph-core/elkLayout.ts). | 30 min | [useGraphLayout.ts](../src/explore/graph-core/useGraphLayout.ts) |
...
⚠️ **Item 9 is an investigation wearing a quick win's clothes.** Its cause is
