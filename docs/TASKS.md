...
The active work. The plan is [TOURS_AND_CONTENT.md](TOURS_AND_CONTENT.md).
 ...
 | `ownership-rules-declarative` | **One declarative place for every classification rule and exception.** [OWNERSHIP_CLASSIFICATION §Where it lives](OWNERSHIP_CLASSIFICATION.md#where-it-lives) makes it look like a big mess: the rules are code in `classifySlotEdge`, the exceptions are five hand-curated sets keyed by slot name, the rule text is a third thing, and the legend groups by yet another. (May be worth looking at `override-site-check` and [BACKLOG §Config rot](BACKLOG.md#hand-curated-config-rot)). **Sketch done 2026-09-11: [OWNERSHIP_RULES_PLAN.md](OWNERSHIP_RULES_PLAN.md)** — one ordered rule array plus a verdict table carrying the drawn appearance, with "can it express `association` as config?" as the acceptance criterion. Blocks step 3 of [`drop-association`](#now--finish-the-tours). | [OWNERSHIP_CLASSIFICATION.md](OWNERSHIP_CLASSIFICATION.md) |
