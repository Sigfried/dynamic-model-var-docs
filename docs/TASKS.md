...
| 3c | **Tour step numbering.** Siggie: *"there needs to be an easy way to find a given tour step/beat as shown in the app in the help-content."* Three numbering schemes describe one walk — the app's `step N of M` counter, the file's step order, and POSITIONS (beats), which outnumber steps: the Walkthrough is ~12 positions across 6 steps. The deleted design note had to spend a paragraph warning that its example was at "step 5 of 6, roughly the 11th `next` press", which is the symptom. | `help-content.md` · `parseHelpContent.ts` |
