...
The active work. The plan is [TOURS_AND_CONTENT.md](TOURS_AND_CONTENT.md).
 ...
 | `ownership-rules` | **Simplify the ownership rules, then rewrite the doc to match.** The plan is [OWNERSHIP_RULES_PLAN.md](OWNERSHIP_RULES_PLAN.md); read it first. Merges what were `drop-association`, `ownership-rules-declarative` and `ownership-doc-cleanup` — they turned out to be one task, because the doc documents the rules and rewriting it before they settle means rewriting it twice. **Plan steps 1–3 and 5 are done** (seven classifier rules collapsed to three, the two deleted override sets gone, `SINGLE_VALUE_OWNER_TARGETS` the only set with members and range-keyed throughout, rules carry human-readable `label`s that the legend shows, the exception nested beneath Rule 2, `required` on `SlotFacts` unused). The classifier runs the rules in TEACHING order and lets an exception reclassify, so there is no second order to derive; `association`'s rule entry is commented out at the foot of the table and must be moved to the front if restored. **What is left is step 4:** OWNERSHIP_CLASSIFICATION.md is 943 lines and has to get much shorter — Siggie wants to be asked about each chunk considered for KEEPING, rather than shown a list of cuts, and wants a draft of the Ownership tour past `why-ownership`/`edge-types` first. `override-site-check` is CLOSED by this work: no set is keyed by slot name any more, so there are no sites to check. | [ownershipRules.ts](../src/models/ownershipRules.ts) |
