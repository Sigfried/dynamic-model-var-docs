...
| 8b | **Draggable panels and popover.** Scoped down 2026-09-08 (Siggie: *"don't do any heavy lifting for the overlays"*): **`HelpPanel.tsx`** is one shared frame that both the legend and the example-cases pane render into, so a drag handle on its header plus CSS `resize: both` makes both draggable and resizable at once; and the **step popover**, which task 8 unblocked — nothing recomputes its position, so a dragged `left`/`top` has nothing to stomp it. No state persistence for now. **Excluded:** the detail drawer (an in-flow flex column — making it an overlay is the layout change that was the bulk of this item) and `TourMap` (a centred modal over a dimmed page that closes on pick; dragging it means nothing). | ~0.5 day | [BACKLOG §Overlays](BACKLOG.md#overlays-one-model-draggable-and-resizable) |
