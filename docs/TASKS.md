...
| 8b | **Overlays: one model, draggable and resizable.** The detail drawer is an in-flow flex column while the legend and cases float, so the legend covers the drawer by construction. Make the drawer an overlay and make overlays draggable/resizable. ⚠️ **The "do task 8 first" ordering was wrong** — re-examined 2026-09-08. Dragging needs the popover's position computed ONCE instead of on every poll tick, which is small and independent of the CSS migration; what genuinely needs task 8 is the RING, which must keep tracking its anchor through a drag. Split those two consumers of `rect` and dragging is reachable first. | [BACKLOG §Overlays](BACKLOG.md#overlays-one-model-draggable-and-resizable) |
