...
## Then — cleanups with a known shape
  ...
  | 13 | **Fix `EXPLORE_VIZ.md`** — the named errors are fixed (2026-09-05); what remains is the vertical-position language that survived the switch to a horizontal default, and the shipped features the doc never mentions (dragging, the example-cases pane, the legend).         | [BACKLOG §EXPLORE_VIZ](BACKLOG.md#explore_vizmd--what-still-needs-fixing)                                                             |
